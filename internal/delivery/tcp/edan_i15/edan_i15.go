...
func (h *Handler) handleMessage(ctx context.Context, message string) (string, error) {
	...
	// Clean up the message for EDAN I15
	...
	slog.Info("EDAN I15: Cleaned message received", "message", logMsg)
	...
	if err != nil {
		slog.Error("EDAN I15: Header decode failed", "error", err, "message_length", len(message))
		return "", fmt.Errorf("decode header failed: %w", err)
	}

	switch m := header.(type) {
	case h251.ORU_R01:
		slog.Info("EDAN I15: Processing ORU_R01 message")
		return h.ORUR01(ctx, m, msgByte)
	}

	slog.Error("EDAN I15: Unknown message type", "type", fmt.Sprintf("%T", header))
